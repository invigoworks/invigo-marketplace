...
도메인과 이름이 결정된 후, 기존 리뷰 파일 존재 여부를 확인한다.

1. `docs/{domain}/reviews/REVIEW_{name}.md` 파일을 Glob으로 탐색한다.
2. **파일이 존재하면 → 재리뷰 모드**:
   - 파일을 Read로 읽는다.
   - `## 조치 필요 항목` 테이블을 파싱한다:
     - `⬜` 상태 항목 → `previousOpenItems` (이전 미조치 항목)
     - `✅` 상태 항목 → `previousResolvedItems` (이전 해결 항목)
   - 헤더의 "N차 리뷰" 번호를 추출한다 → 현재 리뷰 = N+1차
   - "N차 리뷰" 표기가 없으면 이전 리뷰를 1차로 간주 → 현재 = 2차
   - **이전 리뷰 일시**를 헤더의 `리뷰 일시` 값에서 추출한다.
3. **파일이 미존재 → 초회 리뷰**: 기존 동작 그대로 수행한다.
...
| code-reviewer | `코드품질` |
| test-engineer | `테스트` |
| backend-architect | `백엔드아키텍처` |
| architect-reviewer | `아키텍처일관성`, `플랜불일치` |

매핑에 해당하지 않는 분류의 항목은 4개 에이전트 **모두**에게 전달한다.
...
>
> **조건부: 재리뷰 모드 (이전 리뷰 결과가 제공된 경우)**
> - 이전 미조치 항목 각각의 위치를 Read로 확인하여 수정 여부를 판단하라.
> - 수정 확인 → `✅ 해결됨 — [변경 내용 요약]`
> - 미수정 → `⬜ 미해결`
> - 이전에 발견된 항목을 새 발견으로 중복 지적하지 마라.
> - 새로운 문제 + 회귀(regression) 탐지에 집중하라.
> - 상세 리뷰에 `#### 이전 항목 확인 결과` 소섹션을 추가하라.
>
> **이전 미조치 항목:**
> {에이전트 분류별 필터링된 마크다운 테이블}
...
**리뷰 일시**: YYYY-MM-DD (재리뷰 시: N차 리뷰)
**이전 리뷰 일시**: YYYY-MM-DD (재리뷰 시에만 포함)
...
> 이 섹션은 재리뷰 모드에서만 포함됩니다.

| # | 이전 지적 | 조치 결과 |
|---|----------|----------|
| 1 | ❌ [이전 지적 내용] | ✅ [조치 내용] 또는 ⬜ 미조치 |

---

## 조치 필요 항목

| # | 심각도 | 분류 | 위치 (`파일:라인` — `함수명`) | 내용 | 상태 | 비고 |
|---|--------|------|-------------------------------|------|------|------|
| 1 | 심각/중간/낮음 | 코드품질/테스트/아키텍처/플랜불일치 | `파일경로:라인` — `Class.method()` | 구체적 설명 | ⬜/✅ | N차 신규 / N차에서 해결 / 1차 미조치 유지 |

> 분류: 코드품질, 테스트, 백엔드아키텍처, 아키텍처일관성, 플랜불일치
>
> **조치 필요 항목 통합 규칙 (재리뷰 시)**:
> - 이전 해결 항목: `✅` 상태 유지, 비고에 "N차에서 해결"
> - 이전 미해결 항목: `⬜` 상태 유지, 비고에 "1차 미조치 유지" (또는 해당 차수)
> - 신규 발견: `⬜` 상태, 비고에 "N차 신규"
> - 초회 리뷰 시에는 비고 컬럼 생략 (기존과 동일)
...
**재리뷰 시 추가 안내**:
```
재리뷰 결과: 이전 N건 중 M건 해결, K건 미해결. 신규 발견 J건.
```