...
| 변수 | 기본값 |
|------|--------|
| `JWT_ALLOWED_AUDIENCES` | `bitda-api-server,bitda-admin-app,bitda-liquor-app,bitda-tax-app,bitda-manufact-app` |
...
| `POSTGRES_PASSWORD` | `.env.jenkins`의 `POSTGRES_PASSWORD` 값 (하드코딩 금지) |

> **🔐 크리덴셜**: `POSTGRES_PASSWORD` 등 비밀값은 저장소 루트 `.env.jenkins`(git-ignored)에서 로드한다.
> 실행 전 `set -a; . /Users/gimjinhyeog/Desktop/coding/bitda-back/.env.jenkins; set +a`.
