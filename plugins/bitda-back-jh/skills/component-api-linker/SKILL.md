...
> **⚠️ 타협 불가능한 규칙**: 필드 매핑은 **반드시 Notion `table` 블록**으로 작성한다.
> - ❌ `bulleted_list_item` 사용 금지
> - ❌ `paragraph` 로 텍스트 나열 금지
> - ❌ 마크다운 스타일 텍스트 테이블 금지
> - ✅ `"type": "table"` 블록만 사용
...
> **⚠️ 타협 불가능한 규칙**: 변경이력도 **반드시 Notion `table` 블록**으로 작성한다.
> - ❌ `bulleted_list_item` 사용 금지
> - ✅ `"type": "table"` 블록만 사용
  ...
  "table": {
    ...
    "children": [
      ...
      },
      {
        ...
        "table_row": {
          "cells": [
            [{"type": "text", "text": {"content": "2026-02-06"}}],
            [{"type": "text", "text": {"content": "최초 필드 매핑"}}]
          ]
        }
      }
    ]
