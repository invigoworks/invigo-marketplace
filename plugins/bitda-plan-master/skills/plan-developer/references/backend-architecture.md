...
## 7. 기획자를 위한 백엔드 설계 체크리스트

> `backend-planner` 스킬(bitda-back) 기반의 기획서 작성 실전 가이드입니다.
> PART 2 비즈니스 규칙 / PART 3 API 맵핑 작성 시 이 체크리스트를 사용하세요.

### 7.1 데이터 모델 필수 항목

새 엔티티(Aggregate)를 정의할 때 반드시 포함해야 할 항목:

| 항목 | 설명 | 누락 시 영향 |
|------|------|------------|
| `organizationId` | 멀티테넌트 격리 키 | 조직 간 데이터 혼용 버그 |
| `created_at` / `updated_at` | 감사 타임스탬프 | 감사 추적 불가 |
| `version` | 낙관적 잠금 | 동시 수정 충돌 미처리 |
| 삭제 방식 명시 | 소프트 딜리트(`deleted_at`) vs 하드 딜리트 | 삭제 방식 혼란 |
| UNIQUE 제약 조건 | 중복 방지 컬럼 조합 | 중복 데이터 허용 버그 |

### 7.2 비즈니스 규칙 태그 분류

비즈니스 규칙을 다음 4가지 태그로 분류하여 작성합니다 (`backend-planner` 스킬과 연동):

| 태그 | 의미 | 예시 |
|------|------|------|
| `[검증]` | 입력값 제약 (Domain의 require/check) | [검증] 수량은 0보다 커야 함 |
| `[상태]` | 상태별 제약 | [상태] CANCELLED 상태에서는 수정 불가 |
| `[중복]` | 유니크 제약 | [중복] 같은 날짜+품목 중복 등록 불가 |
| `[이벤트]` | 이벤트 트리거 조건 | [이벤트] 불량률 5% 초과 시 품질팀 알림 |

### 7.3 API 설계 컨벤션

| 항목 | 규칙 |
|------|------|
| URL 패턴 | REST 복수형 명사 `/api/v1/{도메인}/{리소스(복수형)}` |
| 목록 조회 | `page` / `size` 쿼리 파라미터 **필수** |
| 단건 생성 응답 | 생성된 UUID 반환 `{ "data": "uuid", "error": null }` |
| PATCH vs PUT | 부분 수정은 PATCH, 전체 교체는 PUT |
| 삭제 방식 | 소프트 딜리트(deleted_at) vs 하드 딜리트 기획서에 명시 필수 |

### 7.4 도메인 이벤트 필요 여부

다음 조건 해당 시 기획서 비즈니스 규칙에 `[이벤트]` 태그로 명시:

- [ ] 다른 모듈(재고, 회계, 품질 등)에 데이터 연동이 필요한가?
- [ ] 특정 조건 충족 시 알림/메시지가 발송되어야 하는가?
- [ ] 외부 시스템(세무서, ERP 등)에 자동 전송이 필요한가?
- [ ] 감사 이력(Audit Trail)이 필요한가?
