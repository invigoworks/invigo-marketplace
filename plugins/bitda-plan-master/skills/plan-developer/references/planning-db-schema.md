...
[로그 저장용] ← 사용자가 수동으로 이동 (Claude 접근 금지)
   (to_do)
```
...
| 로그 저장용 | `로그 저장용` | to_do | 아카이브 (레거시/개인 자료/개발 완료) | **없음 - 최종 상태** |

> **🚫 CRITICAL - "로그 저장용" 접근 금지 규칙**:
> - Claude는 어떤 문서도 "로그 저장용" 상태로 **변경하면 안 됩니다**
> - Claude는 "로그 저장용" 상태의 문서를 **수정하면 안 됩니다**
> - Claude는 "로그 저장용" 상태의 문서를 다른 상태로 **이동하면 안 됩니다**
> - 이 상태는 사용자가 직접 관리합니다 (레거시 문서, 개발 전 개인 기획 자료, 개발 완료 항목)
>
> **단 하나의 예외**: `plan-developer-git` 스킬이 git 이행(Mode 1·5)을 완료한 시점에,
> 해당 페이지에 `notion_legacy_url` frontmatter가 있는 경우에 한해 **status를
> "로그 저장용"으로 1회 전환**할 수 있습니다. 이 외 어떤 도구·플로우도 이 상태 값으로
> 변경 금지. 전환 시 사용자에게 명시적 결과(성공/실패)를 출력해야 합니다.
...
> **⚠️ CRITICAL**: Select 옵션값은 반드시 번호 접두사(`01.`, `02.` 등)를 포함한 **정확한 값**을 사용해야 합니다.
> 존재하지 않는 값을 넣으면 Notion이 새 옵션을 자동 생성하여 DB가 오염됩니다.

| 컬럼명 | 타입 | 필수 | 설명 | 옵션 (정확한 Notion 값) |
|--------|------|------|------|------------------------|
| 사업팀 검토 | Select | - | 사업팀 검토 상태 | `01.대기` / `02.의견있음` / `03.의견 반영 -> 재검토 요청` / `04.승인 완료` |
| BE 검토 | Select | - | 백엔드팀 검토 상태 | `01.대기` / `02.기술검토필요` / `03.의견 반영 -> 재검토 요청` / `04.승인 완료` |
| FE 검토 | Select | - | 프론트엔드팀 검토 상태 | `01.대기` / `02.UI구현검토필요` / `03.의견 반영 -> 재검토 요청` / `04.승인 완료` |
...
```
01.대기 → 02.[부서별 다름] → 03.의견 반영 -> 재검토 요청 → 04.승인 완료
```

**⚠️ 02번 값이 부서마다 다릅니다:**

| 단계 | 사업팀 검토 | BE 검토 | FE 검토 |
|------|-----------|---------|---------|
| 01 | `01.대기` | `01.대기` | `01.대기` |
| 02 | `02.의견있음` | `02.기술검토필요` | `02.UI구현검토필요` |
| 03 | `03.의견 반영 -> 재검토 요청` | `03.의견 반영 -> 재검토 요청` | `03.의견 반영 -> 재검토 요청` |
| 04 | `04.승인 완료` | `04.승인 완료` | `04.승인 완료` |

- 신규 문서 생성 시 초기값: 3개 부서 모두 `01.대기`
- **절대 `대기`, `승인완료` 등 번호 없는 축약값을 사용하지 말 것** — Notion이 새 옵션을 자동 생성하여 DB 오염
- **02번 값을 혼동하지 말 것** — 사업팀은 `의견있음`, BE는 `기술검토필요`, FE는 `UI구현검토필요`
...
### Status 값
- 정확한 옵션명 사용: `"기획 초벌"`, `"부서 협의중"`, `"기획 확정"`, `"기획 변경"`, `"로그 저장용"`
- **🚫 "로그 저장용" 상태는 Claude가 설정/해제하면 안 됨** (사용자 전용)
