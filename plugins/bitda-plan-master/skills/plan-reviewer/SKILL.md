...
- **멀티테넌트 격리**: 데이터 모델에 `organizationId`가 포함되어 있는지
- **삭제 방식**: 소프트 딜리트(`deleted_at`) vs 하드 딜리트가 명시되지 않으면 구현자가 임의 결정
- **목록 API 페이지네이션**: `page`/`size` 파라미터 미명시 시 전체 조회로 구현될 위험
- **UNIQUE 제약**: 중복 방지 조건(`[중복]` 규칙)이 없으면 DB 레벨 제약 누락
- **도메인 이벤트**: 다른 모듈(재고, 회계 등)에 영향 주는 액션에 이벤트 정의 누락 시 연동 버그
- **비즈니스 규칙 분류**: `[검증]`/`[상태]`/`[중복]`/`[이벤트]` 태그 없이 산문으로만 작성 시 구현 누락 위험
