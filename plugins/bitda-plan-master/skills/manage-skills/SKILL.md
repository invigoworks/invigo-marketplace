...
| 스킬 | 설명 | 커버 파일 패턴 |
|------|------|---------------|
| `verify-ui-patterns` | UI 컴포넌트 사용 일관성 검증 (숫자 컬럼 정렬 포함) | `apps/*/src/**/*.tsx`, `packages/web-platform/**` |
| `verify-badge-consistency` | StatusBadge 사용 일관성 및 뱃지 타입 안전성 | `apps/*/src/**/constants.ts`, `apps/*/src/**/types.ts`, `apps/*/src/**/*.tsx` |
| `verify-production` | 생산관리 모듈 구조 검증 | `apps/liquor/src/production/**` |
| `verify-routing` | 라우팅 및 네비게이션 검증 | `apps/*/src/router.tsx`, `apps/*/src/components/layout/**` |
| `verify-plan-implementation` | 기획문서 요구사항의 구현 반영 검증 | `apps/*/src/**`, `packages/core/*/src/**` |
| `verify-plan-changes` | 기획 변경사항의 코드 전파 검증 | `apps/*/src/**`, `packages/core/*/src/**` |
| `verify-dialog-safety` | Dialog/Sheet 상호작용 안전성, state 초기화, readOnly 처리, UX 일관성 검증 | `apps/*/src/**/*Sheet.tsx`, `apps/*/src/**/*Dialog.tsx`, `apps/*/src/**/page.tsx` |
| `verify-data-consistency` | 타입-스키마 동기화, mock 데이터 비즈니스 규칙, 속성 레벨 정합성 검증 | `apps/*/src/**/types.ts`, `apps/*/src/**/validation.ts`, `apps/*/src/**/mock-data/**` |
...
**참조:** `.claude/shared-references/skill-update-rules.md`를 Read 도구로 로드하여 공통 규칙을 확인합니다.

모든 스킬 생성/수정 시 공통 규칙을 준수해야 하며, 핵심 요약은 다음과 같습니다:

- **SKILL.md 500줄 이하** — 초과 시 references/로 분리 (progressive loading)
...
- **중복 금지** — 동일 규칙이 2개 이상의 스킬에 존재하면 안 됨
- **구체적 규칙만** — grep/glob으로 탐지 가능한 규칙만 작성
- **점진적 추가** — 한 번에 3개 이하의 새 규칙 추가
